...
> **The quality bar.** Unless the user gave a widget-by-widget spec or a reference to replicate, a generic ask ("create a report for X") means a **multi-tab report that tells a story, where every tab is a full composed page**. Two page shapes clear the bar — **dashboard tabs** (two or more header-led sections, typically 8–14 widgets) and **editorial tabs** (a narrative chapter: styled opener, 3–8 larger widgets arguing one point, a closing takeaway) — and a report may mix them. What never clears it: token widgets adrift in an empty grid, pages with no narrative framing, every tab shaped the same. Full rules in "Building a report when the request doesn't specify structure" and the pre-handover checklist below, plus `whatagraph-widgets` → "Composing a full tab".
...
- **Every tab must be a full page.** A tab holding one lone table, or a strip of KPIs plus a single chart floating in empty grid, reads as unfinished. Each themed tab gets a complete composition in one of two shapes — a **dashboard tab** (two or more header-led sections of KPIs, charts, and detail) or an **editorial tab** (a narrative opener, a smaller set of larger widgets developing one point, a closing takeaway) — per `whatagraph-widgets` → "Composing a full tab". If a theme can't fill a page from the available data, merge it into a related tab instead of shipping a thin one.
- **Order tabs like a story, and name them for the reader.** The report reads front to back like an argument: **a headline/hook first** (the overview — what happened and why it matters), **evidence chapters next** (themed deep-dives, each answering one question), **the most granular detail late** (long per-entity tables), and — when the report argues a point — **a closing "what it means" tab** that states the conclusion and the next moves. Tab names are short, client-readable labels — "Overview", "Conversions", "Audience" — not sentences or metric lists; an editorial report may number them as chapters ("01 · The headline", "02 · Where the budget went", …, "06 · What it means") so the sequence reads as deliberate. (An "Overview" *tab* inside a report is unrelated to the standalone **Overview / Measurement** entity — when a user's prompt says "overview" outside a clear report-tab context, disambiguate before building; see `whatagraph-mcp-overview` → the "Overview is overloaded" callout and `whatagraph-overviews`.)
...
- **The same bar for every data layer.** None of this depends on where the numbers come from: a single ad channel, GA4, a multi-channel blend or source group, a Google Sheets source, or offline (manual-data) widgets holding hand-entered figures all get the same story-shaped, full-page treatment. An offline or spreadsheet-backed report binds its exhibits via `rows[].data` or the sheet's columns instead of an integration catalog — the composition, narrative, and titling rules don't change (for the offline build order and its PDF traps, load `whatagraph-offline-reports`; its "a document, not a dashboard" framing is editorial mode by another name).
...
1. **Every tab is a full page** — a dashboard tab (headed sections, varied widget mix) or an editorial tab (narrative opener, substantial widgets, closing takeaway); no tab with just one or two token widgets, no half-empty grid below the last row, and the tabs read as a story front to back.
...
4. **Layout is clean and text fits its box** — rows fill their width, no gaps or overlaps, no stranded widget with empty columns beside it; every comment's copy is within its widget's text budget (`whatagraph-widgets` → Comment sizing) — overflow gives no API signal, so count the text, not the error.
...
**The per-tab floor is absolute.** However the tab list came about — user prompt, agent instructions, or your own theming — **no tab ships with one or two token widgets in an empty grid.** A named tab is a request for a complete themed page in one of the two shapes (`whatagraph-widgets` → "Composing a full tab"): a dashboard tab of 8–14 widgets across headed sections, or an editorial tab of 3–8 substantial widgets wrapped in a narrative opener and takeaway — either way the page is full. If a named tab's theme genuinely can't sustain a full page from the available fields, don't ship it thin: merge it into a related tab, or — when the tab list was an explicit instruction — build the fullest page the data supports and tell the user which tab is data-constrained and why.
