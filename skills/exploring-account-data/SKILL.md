...
- When narrowing sources by `channels`, resolve integration ids from `list-integrations` for **this** team — don't hardcode a channel id from memory. On `list_dimensions_and_metrics`, prefer `filter`/`is_universal` to narrow (`filter` supports comma-separated terms with OR logic, e.g. `filter: "cost,clicks"`), and use `page.has_more` (not `estimated_total`, often `null`) to decide whether to paginate.
...
- `resolve_fields` works well on some integrations (Facebook Ads, social channels) but may return empty results on others (e.g., Google Ads). The response includes `semantic_search_supported: bool` — when `false`, the integration has no semantic embeddings and `resolve_fields` will never return results. Fall back to `list_dimensions_and_metrics` with `filter` — this works universally across all integrations.
