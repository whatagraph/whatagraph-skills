...
| `axes.x` / `axes.y` | Intent only: `category`, `value`, `time`, `log`, plus an optional `name`. |
...
| Radar, treemap, sankey | Need a non-cartesian data shape the compiler does not build | Bar or heatmap comparison |
