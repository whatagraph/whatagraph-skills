...
- **Numbers the user pushes in** — the Custom API, a source the user fills themselves, for a system Whatagraph has no connector for. Ongoing and daily, where an offline widget is a fixed set of values typed once. See `whatagraph-custom-api`.
...
4. **Confirm destructive actions — the server now enforces this.** A destructive action refuses its first call and answers with a preview of what it would destroy, carrying a single-use `confirm_token`. Resend the identical call with that token to execute it. A token you were not handed cannot work, so retrying a refused call only earns another preview. This covers every `delete-*` / `remove-*` call and the destructive `manage-*` actions listed in `whatagraph-deleting`. The token is not a substitute for asking: tell the user what the preview says and get their agreement before you confirm. Recovery varies: widgets and report tabs have a `restore` action, reports and spaces are restorable by support within a retention window, custom metrics/dimensions, source groups, and snapshots are permanently gone. Load `whatagraph-deleting` before any delete or removal.
...
6b. Look at what you built with `preview-report` before you report it finished, when that tool is available to you. It returns the pages as images (see `whatagraph-export`). A report can read as correct in the config and still render with a clipped title or a table missing its last row.
7. **Only when the user explicitly asks for it** — share the report (`manage-sharing create`) or schedule delivery (`manage-automations create`). Building a report does not imply sharing it: never create a share link or an automation unprompted. (Note: exporting is **not** sharing — `export-report` is read-only and leaves a report exactly as shared or unshared as it was, so never create a share link in order to produce a PDF or a spreadsheet. A PDF's layout is **fixed and not configurable**: landscape, 1440 CSS px wide, one page per visible tab, each page as tall as that tab's own content — so do not promise a page size, an orientation, or a tab split. Pass `tab_id` for a single tab. Rendering is queued, so the first call returns a job id, not the file; call again with that id to collect it. See `whatagraph-export`.)
