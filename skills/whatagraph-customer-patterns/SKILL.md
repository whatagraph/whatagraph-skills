...
```
Do the instructions explicitly ask for auto-sync (one template kept in step across many client reports)?
...
- **Defaulting to a template (or the pre-made gallery) when no one asked** — open report requests are scratch builds; templates are opt-in, and template-created reports ship unlinked unless auto-sync was explicitly requested.
